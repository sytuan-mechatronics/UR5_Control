"""Manual Phase-2 test tool (motion + vision, no physical grip).

This script runs PickPlaceCycle with experiment_stage=2 so the behavior
matches the production API flow while staying in phase-2 constraints.
"""

import argparse
import json
import os
import socket
import sys
import time
import traceback
from pathlib import Path
from typing import Optional

import numpy as np

ROOT = Path(__file__).resolve().parents[1]
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

import config
from core.job_store import JobStore
from core.pick_place import PickPlaceCycle
from vision.detector import Detector


def check_tcp_port(host: str, port: int, timeout_s: float = 1.0) -> bool:
    """Return True if TCP port is reachable."""
    try:
        with socket.create_connection((host, port), timeout=timeout_s):
            return True
    except Exception:
        return False


def run_preflight(robot_ip: str) -> int:
    """Validate runtime prerequisites without moving robot."""
    print("\n=== PREFLIGHT PHASE 2 ===")

    model_path = config.YOLO_MODEL_PATH
    model_exists = os.path.isfile(model_path)
    print(f"YOLO_MODEL_PATH: {model_path}")
    print(f"Model exists: {model_exists}")
    if not model_exists:
        print("Loi: khong tim thay model. Dung lai preflight.")
        return 1

    try:
        detector = Detector(
            model_path=model_path,
            confidence=config.YOLO_CONFIDENCE,
            target_class=config.YOLO_TARGET_CLASS,
        )
        img = np.zeros((640, 640, 3), dtype=np.uint8)
        detections = detector.detect(img)
        print("Detector load: OK")
        print(f"Detector infer test: OK (detections={len(detections)})")
    except Exception as exc:
        print(f"Detector load/infer FAIL: {exc}")
        return 1

    robot_ports = [
        ("Dashboard", config.DASHBOARD_PORT),
        ("URScript", config.URSCRIPT_PORT),
        ("RTDE", config.RTDE_PORT),
    ]
    for name, port in robot_ports:
        ok = check_tcp_port(robot_ip, port)
        print(f"{name} port {port}: {'OK' if ok else 'FAIL'}")
        if not ok:
            print(f"Loi: khong ket noi duoc {name} port {port}. Kiem tra robot IP va trang thai nguon.")
            return 1

    try:
        from vision.femto_camera import FemtoCamera
        camera = FemtoCamera(width=config.CAMERA_WIDTH, height=config.CAMERA_HEIGHT)
        camera.connect()
        try:
            rgb, depth, _ = camera.get_frames_with_timestamp()
            print(f"Camera stream: OK (rgb={rgb.shape}, depth={depth.shape})")
        finally:
            camera.disconnect()
    except Exception as exc:
        print(f"Camera stream: FAIL ({exc})")
        return 1

    print("Preflight thanh cong. Co the chay test Phase 2 that.")
    return 0


def confirm(message: str, force: bool) -> None:
    """Interactive safety confirmation unless --yes is provided."""
    if force:
        return
    answer = input(f"\n[XAC NHAN] {message}\nTiep tuc? [y/N]: ").strip().lower()
    if answer not in ("y", "yes"):
        print("Da huy test Phase 2.")
        raise SystemExit(0)


def print_summary(job_snapshot: dict, result: Optional[dict]) -> None:
    """Print concise run summary for operator validation."""
    print("\n=== KET QUA PHASE 2 ===")
    if result is not None:
        print(f"status: {result.get('status')}")
        print(f"stage: {result.get('stage')}")
        print(f"detected_objects: {result.get('detected_objects')}")
        target_pose = result.get("target_pose")
        if target_pose is not None:
            print("target_pose:")
            print(json.dumps(target_pose, ensure_ascii=True))
        else:
            print("target_pose: None (khong tim thay target hop le)")

    print("\n=== JOB SNAPSHOT ===")
    print(f"job_id: {job_snapshot.get('job_id')}")
    print(f"status: {job_snapshot.get('status')}")
    print(f"phase: {job_snapshot.get('phase')}")
    print(f"parts_found: {job_snapshot.get('parts_found')}")
    print(f"parts_picked: {job_snapshot.get('parts_picked')}")
    if job_snapshot.get("error"):
        print(f"error: {job_snapshot.get('error')}")


def print_logs(job_snapshot: dict, tail: int) -> None:
    logs = job_snapshot.get("log") or []
    if not logs:
        print("\nKhong co log nao duoc ghi.")
        return
    print(f"\n=== LOG CUOI ({min(len(logs), tail)} dong) ===")
    for line in logs[-tail:]:
        print(line)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Test Phase 2: robot motion + vision processing"
    )
    parser.add_argument(
        "--robot-ip",
        default=config.ROBOT_IP,
        help="Robot IP (default from config.ROBOT_IP)",
    )
    parser.add_argument(
        "--station",
        default="phase2_manual_test",
        help="Station name for generated job",
    )
    parser.add_argument(
        "--workflow-id",
        default=f"manual-phase2-{int(time.time())}",
        help="Workflow ID for generated job",
    )
    parser.add_argument(
        "--yes",
        action="store_true",
        help="Skip interactive safety confirmation",
    )
    parser.add_argument(
        "--log-tail",
        type=int,
        default=40,
        help="Number of latest log lines to print",
    )
    parser.add_argument(
        "--preflight",
        action="store_true",
        help="Only run non-motion checks (model, camera, ports)",
    )
    return parser.parse_args()


def main() -> int:
    args = parse_args()

    print("=== Phase 2 Manual Test ===")
    print(f"Robot IP: {args.robot_ip}")
    print("Mode: motion + camera + YOLO + pick pose transform")
    print("Grip action: disabled in phase 2")

    if args.preflight:
        return run_preflight(args.robot_ip)

    confirm("Robot o trang thai an toan, workspace clear, san sang chay", args.yes)

    job_store = JobStore()
    job = job_store.create_job(
        station=args.station,
        workflow_id=args.workflow_id,
        experiment_stage=2,
    )
    job_id = job["job_id"]

    print(f"\nDa tao job: {job_id} (experiment_stage=2)")

    cycle = PickPlaceCycle(
        robot_ip=args.robot_ip,
        job_store=job_store,
        job_id=job_id,
    )

    result = None
    exit_code = 0
    try:
        result = cycle.run()
    except Exception as exc:
        exit_code = 1
        print("\nPhase 2 test THAT BAI")
        print(f"Loi: {exc}")
        traceback.print_exc()

    job_snapshot = job_store.get_job(job_id) or {}
    print_summary(job_snapshot, result)
    print_logs(job_snapshot, max(1, args.log_tail))

    if exit_code == 0:
        print("\nPhase 2 test HOAN TAT.")
    return exit_code


if __name__ == "__main__":
    raise SystemExit(main())
