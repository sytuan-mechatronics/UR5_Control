...
class URScriptClient:
    ...
    def send_program(self, script_lines: List[str], program_name: str = "pc2_cmd") -> None:
        """
        Send a complete URScript program block.

        Some UR controllers are more reliable when motion commands are sent as
        a full `def ... end` program instead of a single bare statement.
        """
        body = "\n".join(f"  {line}" for line in script_lines)
        script = f"def {program_name}():\n{body}\nend\n"
        logger.debug("Sending URScript program:\n%s", script.rstrip())
    ...
    def move_joint(
        ...
        script = f"movej([{joints_str}], a={accel}, v={vel})"
        logger.info(f"movej to joints: {joints}")
        self.send_program([script], program_name="pc2_movej")
    ...
    def move_linear(
        ...
        script = f"movel(p[{pose_str}], a={accel}, v={vel})"
        logger.info(f"movel to pose: {pose}")
        self.send_program([script], program_name="pc2_movel")
    ...
    def move_joint_to_pose_ik(
        ...
        """
        Move to a Cartesian pose using UR controller inverse kinematics + movej.

        Useful to distinguish:
        - `movel` path/controller issues
        - target pose / TCP / reachability issues
        """
        pose_str = ",".join(f"{p:.6f}" for p in pose)
        script = f"movej(get_inverse_kin(p[{pose_str}]), a={accel}, v={vel})"
        logger.info(f"movej(get_inverse_kin) to pose: {pose}")
        self.send_program([script], program_name="pc2_movej_ik")
    ...
    def set_tcp(self, tcp_pose: List[float]) -> None:
        ...
        script = f"set_tcp(p[{pose_str}])"
        logger.info(f"Setting TCP: {tcp_pose}")
        self.send_program([script], program_name="pc2_set_tcp")
    ...
    def set_payload(self, mass_kg: float, center_of_mass: List[float] = None) -> None:
        ...
        script = f"set_payload({mass_kg}, [{com_str}])"
        logger.info(f"Setting payload: {mass_kg}kg")
        self.send_program([script], program_name="pc2_set_payload")
