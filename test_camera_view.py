...
Chạy:
    ...
    python test_camera_view.py --backend orbbec     # ép dùng Orbbec SDK
...
class OrbbecBackend:
    """Dùng Orbbec binding (`import ob`) để lấy RGB + Depth aligned."""

    NAME = "ob"

    def __init__(self, width=1280, height=720, fps=30):
        import ob                         # noqa: import bên trong để lazy
        self._ob   = ob
        self.width  = width
        self.height = height
        self.fps    = fps
...
def auto_select_backend(force=None, width=1280, height=720):
    ...
    if force == "orbbec":
        print("[Backend] Ép dùng module ob (Orbbec SDK)")
        return OrbbecBackend(width, height)
    ...
    if not diag["sdk_installed"]:
        print("[WARN] Chưa import được module ob (Orbbec SDK) → fallback OpenCV")
        print()
        print("Cài SDK nếu chưa có:")
        print("  python3 -c \"import ob; print('OK')\"")
        print()
        return OpenCVBackend(width, height)
    ...
    for idx, dev in enumerate(diag["devices"], start=1):
        if "error" in dev:
            ...
            print(
                f"  [{idx}] {dev['name']} | SN={dev['serial']} | FW={dev['firmware']}"
            )
    return OrbbecBackend(width, height)
