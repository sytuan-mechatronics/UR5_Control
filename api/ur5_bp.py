...
shared_dashboard: Optional[DashboardClient] = None  # REFACTORED
shared_urscript: Optional[URScriptClient] = None  # REFACTORED
shared_rtde: Optional[RTDEClient] = None  # REFACTORED
shared_gripper: Optional[PneumaticGripper] = None  # REFACTORED
connection_manager = None  # REFACTORED
...
def set_robot_clients(
    ...
    """Receive shared singleton robot clients from app.py."""
    global shared_dashboard, shared_urscript, shared_rtde, shared_gripper, connection_manager
    shared_dashboard = dashboard
    shared_urscript = urscript
    shared_rtde = rtde
    shared_gripper = gripper
    connection_manager = manager  # REFACTORED
...
def _run_job(job_id: str) -> None:
    ...
    try:
        ...
        if connection_manager is None:
            msg = "Robot connection manager is not initialized"
            logger.error(msg)
            job_store.append_log(job_id, msg)
            job_store.update_job(job_id, status="error", error=msg)
            return

        if not connection_manager.ensure_connected():
            msg = "Failed to connect to robot after retries"
        ...
        cycle = PickPlaceCycle(
            ...
            urscript=shared_urscript,
            rtde=shared_rtde,
            job_store=job_store,
            job_id=job_id,
            robot_ip=config.ROBOT_IP,
            gripper=shared_gripper,  # REFACTORED
        )
...
def health() -> Tuple[dict, int]:
    ...
    """
    robot_connection = {"available": False}
    if connection_manager is not None:
        try:
            robot_connection = connection_manager.get_status()
            robot_connection["available"] = True
    ...
    return {
        ...
        "robot_connection": robot_connection,  # REFACTORED
    }, 200
