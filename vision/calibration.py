...
def build_lateral_pre_approach_pose(
    ...
    """
    Build a safer pre-approach pose:
    - move in X/Y toward the target first
    - keep Z exactly at the current TCP Z

    For scan-near-pick workflows this prevents the robot from first moving
    upward because of a noisy target Z estimate.
    """
    pose = [
        point_base[0],
        point_base[1],
    ...
    logger.debug(f"Lateral pre-approach pose: {pose}")
    return pose
